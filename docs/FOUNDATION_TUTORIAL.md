...
校验服务名、地址、连接池、超时
        │
        v
连接 MySQL ──> 执行 001..013 迁移 ──失败──> 退出，不监听 HTTP
        │
        v
...
历史迁移 001..010 被保留，011 做基础升级修正，012 增加好友分页索引和缓存协调事件，013 增加群解散永久墓碑；`backend/scripts/baseline/000_final_schema.sql` 是最终结构的阅读参考，不由程序执行。这样既能升级旧库，也能让新同学快速看懂最终 16 张业务/协调表。
