...
好友、GROUP-001～003 群资料/成员/角色禁言业务已经实现，所以带合法 access 请求 `/friend/list`、`/group/list` 或 `/group/:groupID/members` 会返回真实数据；无 Token 时仍应先返回 401。若要观察“鉴权成功后才进入 TODO”的行为，可改用尚未实现的转让群主接口 `/group/:groupID/owner`、退群接口或消息接口。
