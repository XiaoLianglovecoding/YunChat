...
- [x] **MSG-000 (P0)**：选择并验证消息 ID 生成方案。
  依赖：INFRA-001/002。验收：不能直接使用会在单毫秒超过 1000 条时碰撞的 `millis*1000+seq`；新方案在目标吞吐、多实例、时钟回拨和 Redis 恢复下可证明唯一。
  已冻结规则：私聊和群聊共用 `message_id_allocators(namespace='message')`，事务行锁每次预留一个默认包含 65,536 个 ID 的互斥号段，实例内互斥递增；提交后号段永不回收，崩溃/拒绝只留空洞。ID 不读时钟、不依赖 Redis，Redis 清空/回档或应用重启后继续从持久高水位取新段；上限为 JavaScript 安全整数 `2^53-1`。Lua 接收统一生成器给出的 ID，并独立返回 Redis 毫秒时间，禁止再用 `msgID/1000`。首期“已有号段的进程内热路径”目标 10 万 ID/s，本机最慢基准约 1965 万 ID/s；真实 MySQL 换段延迟留给依赖集成/LOAD-001 持续记录。号段 ID 不承诺跨实例时间有序，Redis `TIME` 也可能回拨；MSG-004 必须为 MQ 晚到、分页并发和回拨增加不回退的同步水位。详见 `docs/MESSAGE_ID_TUTORIAL.md`。
...
- [ ] **MSG-004 (P0)**：断线重连与离线同步。  
  ...
  MSG-000 前置约束：号段 ID 不保证跨实例时间有序，Redis `TIME` 也可能回拨；实现不能直接把当前 `readMessagesAfter` 当作无漏证明，必须增加不回退的持久同步水位/序号或给出等价且覆盖晚到、回拨的协议测试。
...
- [ ] **TEST-003 (P1)**：前后端契约和 E2E。  
  验收：注册→加好友→私聊→建群→离线同步→动态完整链路；当前 44 路由与 TypeScript 类型不漂移。
